...
struct xxx_airlines// struct keyword is used to create a structure

{
   ...
   struct xxx_airlines *following;

}

*begin , *stream;

struct xxx_airlines *dummy;
...

{
...

   {
      ...
      printf("\n\t\t    WELCOME TO XXX AIRLINES RESERVATION SYSTEM     ");
...
begin = stream = (struct xxx_airlines*)malloc(sizeof(struct xxx_airlines));
...
stream->following = (struct xxx_airline *)malloc(sizeof(struct xxx_airlines));
...
FILE *fpointer = fopen("xxx records", "w");
...
printf("\n\n\t Details have been saved to a file (xxx records)");
